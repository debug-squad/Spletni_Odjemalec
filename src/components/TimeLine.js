...
export default function TimeLine() {
    ...
    const steps = [...events.reduce((dates, e) => {
    ...
    return (
        <Box sx={{ width: 'calc(100% - 20px)', backgroundColor: theme.palette.background.default, margin: '10px' }} className='timeline-overlay'>
            <label>Auto next: </label>
            <input type="checkbox" checked={animate} onChange={e=>setAnimate(e.target.checked)} />
            <Stepper nonLinear activeStep={activeStep} sx={{ overflow: 'auto', paddingBottom: '15px'}}>
